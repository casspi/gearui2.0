...
// getInitialState() {
//     let superInit = super.getInitialState();
//     let value = this.getPropStringArrayValue(this.props.value);
//     return G.G$.extend({},superInit,{
...
//         return ele;
//     });
//     return <Select {...props}>{optionsMap}</Select>;
// }
        ...
        // 通过指定的url或者data加载数据
        reload(url:string,dictype:object,method:string,callback?:Function) {
            let __this = this;
            DicUtil.getDic({
                url: url,
                dictype: dictype,
                method:method||"get",
                callback: (dic:any) => {
                    dic = __this._loadFilter(dic);
                    if(dic == null) {
                        __this.triggerChange([]);
                        __this.setState({
                            options: [],
                            value: []
                        });
                    ...
                    let result = __this.doEvent("loadFilter",dic);
                    if(result && result.length > 0) {
                        dic = result[result.length - 1];
                    }
                    let initValue = __this.getInitValue(dic);
                    ...
                    __this.setState({
                        ...
                        if(__this._onLoadSuccess) {
                            __this._onLoadSuccess();
                        }
                        __this.doEvent("loadSuccess");
                    ...
                    __this._onLoadSuccess();
                    __this.doEvent("loadSuccess",dic);
                },
                error: (data:any) => {
                    __this._onLoadError();
                    __this.doEvent("loadError",data);
                }
            });
        }
